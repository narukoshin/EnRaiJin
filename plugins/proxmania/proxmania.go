...
func (p Proxmania) Middleware() middleware.ClientMiddleware {
	return func(next http.RoundTripper) http.RoundTripper {
		return middleware.RoundTripFunc(func(req *http.Request) (*http.Response, error) {
			...
			ctx = context.WithValue(req.Context(), proxy.ProxyContextKey, rp.Proxy)
			req = req.WithContext(ctx)
			resp, err := next.RoundTrip(req)
			if err != nil {
				fmt.Println("fucked up something, ", err)
				return nil, err
			}
			return resp, err
		})
...
func CheckAndFilter() {
	...
	for i := 0; i < len(AliveProxies) && i < cfgParams.ProxyMaxCount; i++ {
...
func Check_WorkingProxies(addr string) ProxyResult {
	client := &http.Client{}
	proxy.Proxy.Addr = addr
	err := proxy.Apply(client)
	if err != nil {
		return ProxyResult{Proxy: addr, Status: StatusDead, ResponseTime: 0, BodyResponse: err.Error()}
	}
	client.Timeout = cfgParams.Timeout
	start := time.Now()
	resp, err := client.Get(proxy.VerifyUrl)
	if err != nil {
		return ProxyResult{Proxy: addr, Status: StatusDead, ResponseTime: 0, BodyResponse: err.Error()}
	}
	...
	if resp.StatusCode == http.StatusOK {
		...
		if err != nil {
			return ProxyResult{Proxy: addr, Status: StatusBad, ResponseTime: duration, BodyResponse: err.Error()}
		}
		return ProxyResult{Proxy: addr, Status: StatusGood, ResponseTime: duration, BodyResponse: string(body)}
	}
	return ProxyResult{Proxy: addr, Status: StatusBad, ResponseTime: duration, BodyResponse: ""}
}
...
func Retrieve_ProxyList() ([]string, error) {
	...
	if cfgParams.ProxyDataSet != nil {
		...
		case []any:
			for _, item := range cfgParams.ProxyDataSet.([]any) {
				// Checking if data set is a local file
				if _, err := os.Stat(item.(string)); err == nil {
					p, err := loadLocalDataSet(item.(string))
					if err != nil {
						return nil, err
					}
					proxies = append(proxies, p...)
					// Checking if data set is a valid url
				} else if isProtocolSchemed(item.(string)) {
					p, err := fetchProxies(client, item.(string))
					if err != nil {
						return nil, err
					}
					proxies = append(proxies, p...)
